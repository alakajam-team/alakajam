    ...
    if (!errorMessage) {
      ...
      if (event.hasChanged("status_results")) {
        if ([enums.EVENT.STATUS_RESULTS.CLOSED, enums.EVENT.STATUS_RESULTS.RESULTS].includes(event.get("status_results"))) {
          await ratingService.computeRankings(event);
          ...
          await ratingService.clearRankings(event);
  ...
  if (!redirected) {
    ...
    if (!event) {
      ...
      if (forms.isId(req.query["event-template-id"])) {
        eventTemplate = await eventTemplateService.findEventTemplateById(forms.parseInt(req.query["event-template-id"]?.toString()));
      }
      event = eventService.createEvent(eventTemplate);
    }

    // Render
    res.render<EventLocals>("event/manage/settings/event-settings", {
      ...res.locals,
      event,
