...
export default function render(context: AdminStatusContext): JSX.Element {
  ...
  return adminBase(context,
    <div>
      ...
      {cacheContents(user, caches)}

    </div>
  );
}

function cacheContents(user: User, caches: Record<string, NodeCache>) {
  if (config.DEBUG_DISABLE_CACHE) {
    return <div>Caching disabled in <code>config.js</code>.</div>;
  }

  return Object.entries(caches).map(([cacheName, cache], index) => {
